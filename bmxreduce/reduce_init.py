...
class reduce(object):
    ...
    def undorfi(self):
        ...
        if np.int(self.tag[0:2]) >= 19:
            ...
            for nm in self.chnames:
                ...
                self.d.datarfi[nm] = (
                    (nsamp-nulled)*clean + nulled*rfival)/nsamp
    ...
    def getind(self):
        ...
        self.ind = {'sc': np.array(sc), 'ec': np.array(
            ec), 'sd': np.array(sd), 'ed': np.array(ed)}
    ...
    def getcal(self):
        ...
        self.ENR = 15  # dB
        # dB, putting noise diode through 2 way splitter,
        self.calAtten = 10*np.log10(1./4)
        ...
        # Mean of surrounding data chunks
        self.dat = np.zeros((self.d.nChanTot, ncal, nf))
        # Mean of surrounding data chunks
        self.Tcal = np.zeros((self.d.nChanTot, ncal, nf))
        ...
        for k in range(ncal):
            ...
            for j in range(self.d.nChanTot):
                ...
                self.Tcal[j, k, :] = np.nanmean(
                    self.Tcalfast[j, sdb:(eda+1), :], 0)
    ...
    def maskdata(self):
        ...
        mask = np.ones((self.nChan, self.d.nSamples,
                        self.f.size)).astype('bool')
    ...
    def getradec(self):
        ...
        point = AltAz(alt=90*u.deg, az=0*u.deg,
                      location=telescope_loc, obstime=time)
    ...
    def downsample(self, dt=5.0):
        ...
        if dt == self.d.deltaT[0]:
            ...
            self.data = np.full(
                (self.nChan, self.d.nSamples, self.f.size), np.nan)
